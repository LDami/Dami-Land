        ...
        public Boolean Save()
        {
            if (editingRace != null)
            {
                ...
                param = new Dictionary<string, object>
                {
                    ...
                    { "@laps", editingRace.Laps },
                    { "@vehicleid", editingRace.StartingVehicle },
                    { "@id", editingRace.Id }
                };
                mySQLConnector.Execute("UPDATE races SET " +
                    "race_name=@name, race_map=@mapid, race_laps=@laps, race_startvehicle=@vehicleid " +
                    "WHERE race_id=@id", param);
                isNew = false;
